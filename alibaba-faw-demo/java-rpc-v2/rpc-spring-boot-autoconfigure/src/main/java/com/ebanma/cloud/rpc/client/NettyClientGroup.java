...
public class NettyClientGroup implements ApplicationContextAware {
    ...
    private void connectProviders() throws Exception {
        ...
        for (String providerName : provders) {

            List<String> providerInstance = zkServer.getChild(providerPath + "/" + providerName);

            if (CollectionUtils.isEmpty(providerInstance)) {
                continue;
            }

            NettyClientBizGroup nettyClientBizGroup = new NettyClientBizGroup(providerName, providerInstance);

            providers.put(providerName, nettyClientBizGroup);
        }
    }
